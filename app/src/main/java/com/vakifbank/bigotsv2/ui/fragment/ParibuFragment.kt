...
class ParibuFragment : Fragment() {
    ...
    private fun sortCoins(sortType: Int) {
        filteredCoins = when (sortType) {
            0 -> filteredCoins.sortedByDescending { kotlin.math.abs(it.paribuDifference ?: 0.0) }
            1 -> filteredCoins.sortedBy { kotlin.math.abs(it.paribuDifference ?: 0.0) }
            2 -> filteredCoins.sortedBy { it.symbol }
            3 -> filteredCoins.sortedByDescending { it.symbol }
            4 -> filteredCoins.sortedByDescending { it.paribuPrice ?: 0.0 }
            5 -> filteredCoins.sortedBy { it.paribuPrice ?: 0.0 }
            else -> filteredCoins
        }
        coinAdapter.submitList(filteredCoins)
    }
